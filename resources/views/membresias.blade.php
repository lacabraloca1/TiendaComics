...
<div id="modalNuevoCliente" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
    <div class="bg-white p-5 rounded shadow-lg text-black w-96">
        ...
        <select id="tipoMembresia" class="w-full p-2 border rounded mb-3" onchange="actualizarMonto()">
            <option value="Básica">Básica</option>
            <option value="Pro">Pro</option>
        </select>

        <label class="block text-gray-700 font-medium">Monto</label>
        <input type="number" id="montoMembresia" class="w-full p-2 border rounded mb-3" readonly>
...
<script>
    ...
    function actualizarMonto() {
        let membresia = document.getElementById('tipoMembresia').value;
        let montoInput = document.getElementById('montoMembresia');

        if (membresia === "Básica") {
            montoInput.value = 250;
        } else if (membresia === "Pro") {
